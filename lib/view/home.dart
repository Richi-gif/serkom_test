// richi-gif/serkom_test/serkom_test-main/lib/view/home.dart
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  // This callback will be triggered when a search is submitted or cleared in SearchBukuWidget
  void _onSearchQueryChanged(String query) {
    setState(() {
      _currentSearchQuery = query;
      if (_currentSearchQuery.isEmpty) {
        _fetchBooks(); // Re-fetch default books if search is cleared
      }
    });
  }
          ...
          child: Container(
            ...
            decoration: const BoxDecoration(
              ...
              image: DecorationImage(
                image:
                    AssetImage("assets/photo-1534528741775-53994a69daeb.webp"),
                fit: BoxFit.cover,
              ),
            ),
          ),
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              // Search bar and its results handled by SearchBukuWidget
              SearchBukuWidget(
                initialQuery: _currentSearchQuery,
                bookService: _bookService,
                onSearchSubmit:
                    _onSearchQueryChanged, // Pass callback for submission
                onClearSearch: () =>
                    _onSearchQueryChanged(''), // Pass callback for clearing
              ),

              // Only show Popular/Best Seller sections if there is no active search query
              if (_currentSearchQuery.isEmpty) ...[
                ...
                FutureBuilder<List<Book>>(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return Center(
                          child: Text(
                              'Error loading popular books: ${snapshot.error}',
                      ...
                      if (booksWithCovers.isEmpty) {
                        return const Center(
                          child: Text(
                              'No popular books with covers found. Try a different search.',
                              style: TextStyle(color: Colors.white70)),
                        );
                      }
                      return Column(
                ...
                FutureBuilder<List<Book>>(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      return Center(
                          child: Text(
                              'Error loading best seller books: ${snapshot.error}',
                              style: const TextStyle(color: Colors.red)));
                    } else if (!snapshot.hasData || snapshot.data!.isEmpty) {
                      return const Center(
                      ...
                      if (booksWithCovers.isEmpty) {
                        return const Center(
                          child: Text(
                              'No best seller books with covers found. Try a different search.',
                              style: TextStyle(color: Colors.white70)),
                        );
                      }
